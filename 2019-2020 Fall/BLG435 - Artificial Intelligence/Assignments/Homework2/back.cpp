...
struct Node {
    ...
    void display() {
        pair<int, int> score = get_scores();
        printf("R: %d,  C: %d, Score: %d - %d, Maximize: %d\n", R, C, score.first, score.second, get_maximize());
        ...
        for (int i=0; i < R; i++) {
            for (int j=0; j < C; j++) {
                printf(get_horizontal_edge(h_index++) ? "+-" : "+ ");
            } printf("+\n");

            for (int j=0; j < C + 1; j++) {
                printf(get_vertical_edge(v_index++) ? "| " : "  ");
            } printf("\n");
        }
