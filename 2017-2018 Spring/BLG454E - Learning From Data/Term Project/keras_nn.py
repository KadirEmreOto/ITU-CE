...
class Classifier:
    ...
    def get_model(self):
        model = Sequential()
        model.add(Dense(512, input_shape=(14,)))
    ...
    def process(self, X, y):
        ...
        delivery_time = self.convert_delivery((X[:, 2] - X[:, 1]) // 86400)
        age = np.array([self.convert_age((datetime.fromtimestamp(x) - datetime.now()).days // 365) for x in X[:, 10]])
